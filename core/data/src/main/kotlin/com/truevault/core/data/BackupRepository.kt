 ...
 *
 * The archive is encrypted *before* it leaves the app, with a key derived from a password the user
 * chooses for the backup — not from the vault password and not from the Keystore. That is the whole
 * point: a backup sealed by a device-bound Keystore key could never be restored anywhere, including
 * on a replacement phone, which is exactly when a backup matters.
 *
 * Cloud sync is not part of this. The repository interface is shaped so a remote destination can be
 * added later without touching the vault engine — it consumes an `OutputStream`, and where that
 * stream goes is the caller's decision.
 */
@Singleton
class BackupRepository @Inject constructor(
    ...
    fun export(destination: Uri, passphrase: CharArray): Flow<BackupStep> = channelFlow {
        ...
        if (items.isEmpty()) {
            send(BackupStep.Failed(VaultError.Unknown("There is nothing in the vault to back up."), null))
            return@channelFlow
        }
            ...
            try {
                context.contentResolver.openOutputStream(destination)?.use { rawOut ->
                    ZipOutputStream(rawOut.buffered()).use { zip ->
                        ...
                        // The vault index travels as one sealed blob, so restoring gets metadata
                        // and files together or not at all.
                        val metadataBytes = json.encodeToString(
                            kotlinx.serialization.builtins.ListSerializer(BackupItemRecord.serializer()),
                            items.map { it.toRecord() },
                        ).toByteArray()
                        ...
                        items.forEachIndexed { index, item ->
                            ...
                            val file = fileSystem.itemFile(item.id)
                            if (file.exists()) {
                                entries += zip.writeSealed(
                            ...
                            if (item.thumbnailRelativePath != null && thumbnail.exists()) {
                                entries += zip.writeSealed(
                                    name = "${BackupManifest.THUMBNAIL_ENTRY_PREFIX}${item.id}",
                                    plaintext = thumbnail.readBytes(),
                                    key = archiveKey,
                                )
                            }
                        }
                    ...
                    send(BackupStep.Failed(VaultError.Unknown("That location could not be written."), null))
                    return@withContext
                }
                ...
                send(BackupStep.ExportFinished(items.size, 0L))
            } catch (e: VaultLockedException) {
                send(BackupStep.Failed(VaultError.AuthenticationRequired, null))
            } catch (e: Exception) {
     ...
     * commits to anything, and to reject an unreadable file with a specific reason.
     */
    suspend fun inspect(source: Uri): Outcome<BackupManifest> = withContext(ioDispatcher) {
        try {
            readManifest(source)?.asSuccess()
     ...
     * Restores an archive.
     *
     * The order is deliberate and matches the import engine's philosophy: validate the manifest,
     * check the format version, check free space, unpack to a staging directory, verify every entry
     * against its recorded hash, and only then commit. A restore that fails part-way leaves the
    ...
    fun restore(source: Uri, passphrase: CharArray): Flow<BackupStep> = channelFlow {
        withContext(ioDispatcher) {
            ...
            val params = KdfParams.forVersion(manifest.kdfVersion) ?: run {
                send(
                    BackupStep.Failed(
            ...
            // Wrong password is detected here, cheaply, instead of as a confusing decryption failure
            // half way through a large restore.
            ...
            if (available < required) {
                send(
                    BackupStep.Failed(
                        VaultError.InsufficientStorage(required, available),
                        null,
                    ),
                )
                return@withContext
            ...
            try {
                val unpacked = unpackAndVerify(source, manifest, archiveKey, staging) { done, total ->
                    trySend(BackupStep.Progress(done, total))
                }

                val records = json.decodeFromString(
                    kotlinx.serialization.builtins.ListSerializer(BackupItemRecord.serializer()),
                    String(unpacked.metadata),
                )
                ...
                records.forEach { record ->
                    ...
                    try {
                        ...
                        vaultItemDao.insert(record.toEntity())
                        restored++
                    } catch (e: Exception) {
                ...
                archiveKey.encoded?.wipe()
            }
        }
    }

    private data class Unpacked(val metadata: ByteArray)

    /**
     * Unpacks every entry into [staging], verifying each one's hash as it goes.
     *
     * Verification happens against the manifest's recorded hash of the *sealed* bytes, so a
     * truncated or edited archive is caught before any of it reaches the vault directory.
     */
    private fun unpackAndVerify(
        ...
        context.contentResolver.openInputStream(source)?.use { rawIn ->
            ZipInputStream(rawIn.buffered()).use { zip ->
                while (true) {
                    ...
                    if (!digest.b64().contentEquals(declared.sha256Base64)) {
                        throw BackupException.Corrupt(name)
                    }
                    ...
                    when {
                        name == BackupManifest.METADATA_ENTRY_NAME -> metadata = plaintext

                        name.startsWith(BackupManifest.ITEM_ENTRY_PREFIX) ->
                            File(staging, name.removePrefix(BackupManifest.ITEM_ENTRY_PREFIX))
                                .writeBytes(plaintext)

                        name.startsWith(BackupManifest.THUMBNAIL_ENTRY_PREFIX) ->
                            File(
                                staging,
        ...
        return Unpacked(metadata ?: throw BackupException.Incomplete(1))
    }
    ...
    private fun ZipOutputStream.writeSealed(
        ...
        return BackupEntry(
            ...
            sha256Base64 = MessageDigest.getInstance("SHA-256").digest(sealed).b64(),
        )
    }
...
/** One vault item as it travels inside a backup. Sealed, never plaintext on disk. */
@kotlinx.serialization.Serializable
internal data class BackupItemRecord(
    val id: String,
    val encryptedMetadataBase64: String,
    ...
    val contentFingerprintBase64: String?,
    val hasThumbnail: Boolean,
)

private fun VaultItemEntity.toRecord() = BackupItemRecord(
    id = id,
    encryptedMetadataBase64 = encryptedMetadata.b64(),
    ...
    contentFingerprintBase64 = contentFingerprint?.b64(),
    hasThumbnail = thumbnailRelativePath != null,
)

private fun BackupItemRecord.toEntity() = VaultItemEntity(
    id = id,
    fileRelativePath = "$id.vault",
    thumbnailRelativePath = if (hasThumbnail) "$id.thumb" else null,
    encryptedMetadata = encryptedMetadataBase64.unB64(),
    mimeCategory = mimeCategory,
    encryptedSize = encryptedSize,
    originalSize = originalSize,
    createdAt = createdAt,
    ...
    fileFormatVersion = fileFormatVersion,
    originalDeletionState = originalDeletionState,
    contentFingerprint = contentFingerprintBase64?.unB64(),
